    ...
    def learning_step(self, trajectory):
        ...
        for _ in range(args.num_ppo_epochs):
            ...
            for b_st in range(0, len(input_ids), args.train_batch_size_per_device):
                ...
                if args.reinforce_update:
                ...
                elif self.args.critic_type_modify == "hmpo":
                    ...
                    # Harmonic mean
                    hm_ratio = mb_response_masks.sum() / (1/ratio)[mb_response_masks].sum()
                    pg_losses = -mb_advantage * hm_ratio
                ...
                with torch.no_grad():
                    if not args.reinforce_update and "hmpo" in self.args.critic_type_modify:
