...
for name, value in data.items():
    if "gmpo_7B_kl3.log" in name:
        name = "GMPO"
        color = (0, 0.8, 0.2)
    elif "grpo_7B_kl3.log" in name:
        name = "GRPO"
        color = (1, 0, 0)
    else:
        continue
    kl3 = [el["train/kl3"] if el["train/kl3"]<300 else 300 for el in value[:max_len] if "train/kl3" in el]
    ...
    steps = list(range(len(kl3)))

    # 绘制平滑后的 logprobs_diff_max 曲线
    plt.plot(steps, kl3, label=f"{name}", color=color, linewidth=3)
...
# 保存图像
plt.savefig('logs/kl3.png', dpi=600, bbox_inches='tight')
plt.show()
